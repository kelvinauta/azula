    ...
    static async set_data(postgres, count = 5) {
        ...
        for (let i = 0; i < count; i++) {
            agents.push({
                ...
                config: {
                    model: faker.helpers.arrayElement(['gpt-4', 'gpt-3.5-turbo', 'claude-2']),
                    temperature: faker.number.float({ min: 0, max: 1 }),
                    settings: {
                        max_tokens: faker.number.int({ min: 100, max: 2000 }),
                        context_length: faker.number.int({ min: 1000, max: 8000 })
                    }
                }
            });
        }
    ...
    static async set_data(postgres, count = 10) {
        ...
        for (let i = 0; i < count; i++) {
            prompts.push({
                ...
                config: {
                    type: faker.helpers.arrayElement(['system', 'user', 'assistant']),
                    parameters: {
    ...
    static async set_data(postgres, count = 6) {
        ...
        for (let i = 0; i < count; i++) {
            tools.push({
                ...
                external_id: uuidv4(),
                config: {
                    name: faker.helpers.arrayElement(['calculator', 'translator', 'search', 'weather']),
...
class Human {
    ...
    static async set_data(postgres, count = 10) {
        ...
        for (let i = 0; i < count; i++) {
            humans.push({
                id: uuidv4(),
                type: faker.helpers.arrayElement(['external_user', 'internal_user']),
                info: {
    ...
    static async set_data(postgres, count = 20) {
        ...
        for (let i = 0; i < count; i++) {
            messages.push({
                ...
                files: faker.datatype.boolean() ? { url: faker.internet.url() } : null,
        ...
        for (let i = 0; i < count; i++) {
            ...
            threads.push({
                ...
                logs: {
                    ...
                    status: faker.helpers.arrayElement(['success', 'error', 'timeout'])
                },
